...
| Pinned export lists | `.`: 197 value exports, `./runtime`: 138 value exports; the two sets are disjoint and deep `dist/` paths are not importable | `test/fixtures/public-api/root-exports.json`, `runtime-exports.json` (`test/public-api.test.ts`, `scripts/check-pack.mjs`) |
...
| `WORKFLOW_CONTRACT_REVISION` | 21 | `src/contracts-core.ts` (re-exported by `src/contracts.ts`); 2.0.0 raised it from 18 to 19 for the pi-subagent revision-7 handshake and the optional `memoryBytes` in agent task identity. The unreleased release raises it to 20 because run state moves: runs, leases, prune trash, and dynamic proposals are keyed under `<agentDir>/workflow/<projectKey>` instead of `<cwd>/.pi/workflow`, and the location of persisted state is part of the contract ([docs/persistence.md](persistence.md#where-state-lives)). No record shape changes with it. It later gained one feature flag, `serviceProviderStart: true`, and one optional persisted event field, `run-created`'s `origin`, both additive within the revision: a journal written without `origin` still validates, the reducer does not read it, and no identity derivation changes, so the revision does not advance again for it. A consumer pins the feature flag, not the field. It then advanced again to 21, because `WorkflowRunRecordSchema` gained the optional `ceiling` - the host delegation ceiling a run started under, recorded at creation and lowered onto every `SubagentRequest` the run makes. That is persisted state a reader must understand: a reader that ignored it would misread every launch the run made, which is what the revision rule counts rather than the shape of the field. The revision is an input to `hostApiSha256` by construction, so the dynamic host API digest rotates and every existing dynamic source approval is invalidated; nothing else derives identity from it. Revision-21 stores refuse revision-20 and revision-19 leases, journals, snapshots, run records, decision records, and dynamic proposals; state left under an older root or revision is not seen, and there is no migration, no fallback root, and no dual-root reader |
| `WORKFLOW_HANDOFF_FORMAT.revision` | 7 | `src/contracts.ts`; the pi-subagent revision that DEFINES the handoff rendering, which is not the same as the revision currently required. pi-subagent 0.14.0 is revision 8 and renders a handoff identically, so this stays 7: it is an input to `WORKFLOW_HANDOFF_FORMAT_SHA256`, the `schemaSha256` of every handoff artifact, and moving it would fail every existing handoff artifact's verification for no change in the bytes |
...
| Required `@vegardx/pi-subagent` | `0.14.0` (exact; raised from `0.13.0` by the delegation ceiling) | `package.json` `peerDependencies` |
| Required pi-subagent contract revision | 8 | `WORKFLOW_RUNTIME_CONTRACT.requiredSubagent.contractRevision` |
| pi-subagent commit built in CI | `0ae4c106b4235d8576bde75a23896ba413bcf88e` | `.github/workflows/ci.yml` |
...
it is revision 8 and every feature below has exactly this value.
