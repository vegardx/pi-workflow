...
| Handoff import | Owner client `exportHandoff` rejection, an invalid `HandoffRef`, an identity that differs from the settled `{ attemptId, baselineHead, handoffCommit }`, an unsupported format, a digest or size mismatch, or bytes that are not a single-commit `git format-patch`; a completed worktree child that captured no handoff under `handoff: "required"` | Import failures terminalize the execution `cleanup-blocked` at stage `handoff-import` before any release intent; the task and run become `cleanup-blocked` until `workflow_reconcile` reconciles the child and retries the import. The no-handoff case is released normally and then `failed` at stage `handoff-import` with "Completed worktree task captured no handoff."; under `handoff: "optional"` it completes without a handoff |
| Handoff import bound | The settled handoff exceeds `MAX_WORKFLOW_HANDOFF_BYTES` (16 MiB): pi-subagent refuses the export ("handoff export exceeds byte limit") or returns a reference whose proved length is above the bound, under either handoff policy | A permanent failure, never a cleanup block: the execution terminalizes `failed` at stage `handoff-import` with "Workflow handoff exceeds the import bound.", the task fails with the same reason, a required task fails the run and an optional one degrades it, and `retry`/`invalidate` become legal. The child is left unreleased so pi-subagent keeps protecting its worktree and handoff ref; a re-drive of an already `cleanup-blocked` import supersedes that terminal with the same failure instead of looping |
...
handoff above the import bound:
          … → task-execution-artifact-imported
          → task-execution-terminal (outcome failed, evidence kind workflow,
            stage handoff-import,
            "Workflow handoff exceeds the import bound.")
          → task-status-changed → failed
          → run-status-changed → failed (required task only; an optional task
            leaves the run running and degrades it at the end)

the same refusal proved on a re-drive of a blocked import:
          terminal (cleanup-blocked, handoff-import)
          → task-execution-terminal (outcome failed, stage handoff-import,
            "Workflow handoff exceeds the import bound.") supersedes it
          → task-status-changed cleanup-blocked → failed
          → run-status-changed cleanup-blocked → failed
```
...
handoff is not a single-commit git-format-patch."); the persisted terminal
message is "Workflow handoff artifact import requires reconciliation." and raw
error text is never journaled. The bound is the exception: pi-subagent's fixed
byte-limit refusal (`WorktreeError` "handoff export exceeds byte limit",
matched down the cause chain) and a reference whose proved length exceeds
`MAX_WORKFLOW_HANDOFF_BYTES` both terminalize the execution `failed` at stage
`handoff-import` with the persisted message "Workflow handoff exceeds the
import bound." instead of blocking, because no re-drive can change the size of
a settled commit pair. The child is not released, so pi-subagent keeps the
worktree, its reservation branch, and the handoff ref out of ordinary
retention; the operator exports, pins, or releases it there, and `retry` or
`invalidate` re-executes the task in a fresh worktree.
